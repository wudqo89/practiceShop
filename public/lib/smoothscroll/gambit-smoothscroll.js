...
(function() {
   ...
   */
  GambitSmoothScroll.prototype.newScroll = function(
    isDown,
    scrollableElement,
    timestamp
  ) {
    ...
    if ('undefined' === typeof this.scrollInterval) {
      ...
      this.scrollInterval = setInterval(
        function() {
          // Perform the scroll
          var scrollBy =
            ((this.isDown ? 1 : -1) * this.startingSpeed) / this.settings.force;
      ...
      // Limit the amount
      this.startingSpeed = Math.max(
        this.startingSpeed * multiplier,
  ...
  GambitSmoothScroll.prototype.overrideScroll = function(e) {
    ...
    var scrollPercentage,
      scrollableElement = this.parent_scroll(e.target);
    ...
    if (
      (Math.abs(delta) < 1 || this.startedAsTrackpad) &&
      !skipFirefoxMacCheck
    ) {
